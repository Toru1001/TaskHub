...
<body>
    ...
    <div class="task-container">
        ...
        <div class="tasks-container">
            ...
            $taskresult = mysqli_query($conn, "SELECT * FROM `all_tasks` WHERE `user_id` = '$user_id' AND `assigned_to` = '$username' AND `status` = 'Ongoing' AND priority = 'High';");
            if (mysqli_num_rows($taskresult) > 0) {
                while ($row = mysqli_fetch_assoc($taskresult)) {
                    echo '<div class="task-model">
                <li class="task-detail">';
                    echo '<i class="bx bx-calendar-exclamation icon5"></i>';
        ...
        <div class="tasks-container">
            ...
            <?php
            $username = $fetch['username'];
            $taskresult = mysqli_query($conn, "SELECT * FROM `all_tasks` WHERE `assigned_to` = '$username' AND `status` = 'Ongoing' AND priority = 'High';");
            if (mysqli_num_rows($taskresult) > 0) {
                while ($row = mysqli_fetch_assoc($taskresult)) {
        ...
        <div class="tasks-container">
            ...
            <?php
            $username = $fetch['username'];
            $taskresult = mysqli_query($conn, "SELECT * FROM `all_tasks` WHERE `assigned_by` = '$username' AND `status` = 'Ongoing' AND priority = 'High';");
            if (mysqli_num_rows($taskresult) > 0) {
                while ($row = mysqli_fetch_assoc($taskresult)) {
