    ...
    <div class="task-container">
        ...
        <div class="tasks-container">
            ...
            for ($i = 0; $i < 7; $i++) {
                ...
                $taskresult = mysqli_query($conn, "SELECT * FROM `all_tasks` WHERE `user_id` = '$user_id' AND `assigned_to` = '$username' AND `status` = 'Ongoing' AND `due_date` = '$nextDate';");
        ...
        <div class="tasks-container">
            ...
            for ($i = 0; $i < 7; $i++) {
                $nextDate = date('Y-m-d', strtotime("+$i days"));
                $taskresult = mysqli_query($conn, "SELECT * FROM `all_tasks` WHERE `assigned_to` = '$username' AND `status` = 'Ongoing' AND `due_date` = '$nextDate';");
                if (mysqli_num_rows($taskresult) > 0) {
                    while ($row = mysqli_fetch_assoc($taskresult)) {
        ...
        <div class="tasks-container">
            ...
            for ($i = 0; $i < 7; $i++) {
                $nextDate = date('Y-m-d', strtotime("+$i days"));
                $taskresult = mysqli_query($conn, "SELECT * FROM `all_tasks` WHERE `assigned_by` = '$username' AND `status` = 'Ongoing' AND `due_date` = '$nextDate';");
                if (mysqli_num_rows($taskresult) > 0) {
                    while ($row = mysqli_fetch_assoc($taskresult)) {
